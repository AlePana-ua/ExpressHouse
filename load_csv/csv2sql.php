...
if(isset($_POST['username'])&&isset($_POST['mysql'])&&isset($_POST['db'])&&isset($_POST['username']))
{
    ...
    $db=$_POST['db'];
    $file=$_POST['csv'];
    $cons= mysqli_connect("$sqlname", "$username","$password","$db") or die("Conexion");
    ...
    mysqli_query($cons, 'LOAD DATA LOCAL INFILE "'.$file.'" INTO TABLE '.$table.' FIELDS TERMINATED by \',\' LINES TERMINATED BY \'\n\' IGNORE 1 LINES;') or die(mysqli_error($cons));
    echo "Terminada!";
    //$result2=mysqli_query($cons,"select count(*) count from $table");
    $r2= 0;//mysqli_fetch_array($result2);
    $count2= 0;//(int)$r2['count'];
